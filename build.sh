...
find -type f -name '*.asciidoc' -not -name 'main*' | while read f; do
    ...
    if [ -d "${f%.asciidoc}" ]; then
        ...
        cat "$f" \
        | awk 'BEGIN { base="'"$pathBase"'/" } /^include::/ { file = base substr($0, 10, length($0)-9-2); while (getline line<file != 0) { if (match(line, /^(=+ )(.*)$/, a) > 0) { level=a[1]; title = a[2]; break } }; close(file); print $0 " #!prefix=\"" level "\",title=\"" title "\""; next }; {print}' \
        | sed -r -e "s/^include::(.*)\.asciidoc\[\] #!prefix=\"([^\"]*)\",title=\"(.*)\"$/\2<<$idBase\1,\3>>\n\n--\n--/; :noslash; s/^(=* *<<[^,]*)\//\1-/; t noslash; :end;" \
        | awk 'BEGIN { remove=-1 }; remove == -1 && /^=+ / { remove = index($0, " ") - 1; }; remove > 0 && /^=+ / { print substr($0, remove); next; }; { print }' \
        ...
        cat "$f" \
        | awk 'BEGIN { remove=-1 }; remove == -1 && /^=+ / { remove = index($0, " ") - 1; }; remove > 0 && /^=+ / { print substr($0, remove); next; }; { print }' \
        | asciidoc --backend html5 --attribute toc --attribute disable-javascript -o "$out.pre" -
        ./rewrite.py index.xml "$out.pre" > "$out"
    fi
    rm "$out.pre"
done
